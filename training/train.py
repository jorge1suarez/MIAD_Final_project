...
def train():
    ...
    joblib.dump(ts_scaler, 'ts_scaler_latest') 

    # save train_series
    train_series.to_pickle("train_series_latest")

    # save tsdataset
    training.save("TimeSeriesDataSet_training_latest")
    ...
    torch.save(loaded_model_in_cpu, 'model_latest')

    print("training DONE")

def upload_model():
     # upload to google cloud storage bucket
    upload_blob('miad-bucket', './ts_scaler_latest', 'models/ts_scaler_latest')
    upload_blob('miad-bucket', './train_series_latest', 'models/train_series_latest')
    upload_blob('miad-bucket', './TimeSeriesDataSet_training_latest', 'models/TimeSeriesDataSet_training_latest')
    upload_blob('miad-bucket', './model_latest', 'models/model_latest')
